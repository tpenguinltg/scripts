...
duplicity --full-if-older-than 3M --verbosity info --no-encryption --asynchronous-upload / --exclude-if-present .nobackup --include /etc --include /var/lib/pacman/local --include /boot --exclude '**' "file://$destination/system"
exit_status=$?
...
duplicity --full-if-older-than 3M --verbosity info --no-encryption --asynchronous-upload / --exclude-if-present .nobackup --include /etc --include /var/spool/cron --exclude '**' "file://$destination/crontabs"
exit_status=$?
if [ $exit_status -eq 0 ]; then
...
duplicity --full-if-older-than 3M --verbosity info --no-encryption --asynchronous-upload / --exclude-if-present .nobackup --include /home --include /data --exclude '**' "file://$destination/data"
exit_status=$?
if [ $exit_status -eq 0 ]; then
